...
- Use `Message.match<Update.Return<Model, Message>>` or a reused `UpdateReturn` alias for exhaustive Message matching. Use Effect `Match` for other tagged unions, partial matching, fallbacks, and one handler shared across multiple tags. For exhaustive Effect matches, prefer `Match.tagsExhaustive({ ... })` over `Match.tag(...)` chains. Never use `switch`.
...
- Constrain Match branch returns at the match boundary with `Match.withReturnType<...>()`. This includes tuple literals nested inside Effect or Option constructors. Never use `as const` inside branches to recover tuple or literal inference.
